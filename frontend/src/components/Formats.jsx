            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
                {FORMATS.map((f, i) => (
                    <Reveal key={f.number} delay={i * 0.12} className="h-full">
                        ...
                        >
                            <span className="font-display text-5xl text-stroke group-hover:text-red-600 group-hover:[-webkit-text-stroke:0px] transition-all duration-300">
                                {f.number}
                            </span>
                            <h3 className="font-display text-2xl sm:text-3xl uppercase tracking-wide text-white mt-6 mb-3">
                                {f.title}
                            </h3>
                            ...
                            <ul className="mt-6 pt-6 border-t border-neutral-800 space-y-2.5">
                                {f.includes.map((inc) => (
                                    <li key={inc} className="flex items-start gap-2.5 text-sm text-neutral-300">
                                        <Check size={16} className="text-[#D4AF37] mt-0.5 shrink-0" strokeWidth={2.5} />
                                        {inc}
                                    </li>
                                ))}
                            </ul>
